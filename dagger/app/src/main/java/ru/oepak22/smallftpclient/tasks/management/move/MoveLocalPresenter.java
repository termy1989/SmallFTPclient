...
public class MoveLocalPresenter {
    ...
    // конструктор
    public MoveLocalPresenter(CopyMoveDeleteView view, FilesService filesService,
    ...
    public void move() {
        ...
        if (mService.isSDcard(mStartMoveList.get(0).getSourcePath())
                && mService.isSDcard(mStartMoveList.get(0).getDestinationPath()))
            moveFilesSimple();
        else if (!mService.isSDcard(mStartMoveList.get(0).getSourcePath())
                    && !mService.isSDcard(mStartMoveList.get(0).getDestinationPath()))
            moveFilesSimple();
        else moveFiles();
    ...
    private void moveFilesSimple() {
        ...
        for (OperationFile item : mStartMoveList) {
            ...
            if (!((AsyncTask) mView).isCancelled()) {
                ...
                boolean rename = mService.rename(mContext, new File(item.getSourcePath()),
    ...
    private void moveAllFiles() {
        ...
        // удаление папок
        for (String path : mDeleteFoldersList) mService.delete(mContext, new File(path));
        ...
        try {
            ...
            if (inputFile.exists()) {
                ...
                if (inputStream != null && outputStream != null) {
                    ...
                    // удаление файла
                    mService.delete(mContext, inputFile);
                }
            }
        }
