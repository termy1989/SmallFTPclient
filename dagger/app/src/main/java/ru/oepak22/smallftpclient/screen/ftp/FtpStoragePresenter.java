...
public class FtpStoragePresenter {
    ...
    private final FtpService mService;                                                                  // интерфейс операций FTP
    ...
    // конструктор
    public FtpStoragePresenter(FtpStorageView view, FtpService ftpService,
                                        FtpServer server, FTPClient client) {
        mService = ftpService;
        mView = view;
        mCachedList = null;
        mServer = server;
    ...
    public void refresh() {
        ...
        if (!mService.isActive(mClient)) {

            mSubscription = mService.connect(mClient, mServer, mCurrentDir)
    ...
    protected void refreshConnected() {
        ...
        Observable<String> path = mService.getWorkingDirectory(mClient);
        Observable<List<FTPFile>> files = mService.getFiles(mClient);
    ...
    protected void changeDir(String name) {
        ...
        mSubscription = mService.changeDirectory(mClient, name)
                                .doOnSubscribe(mView::showLoading)
                                .doAfterTerminate(mView::hideLoading)
                                .subscribeOn(Schedulers.io())
                                .observeOn(AndroidSchedulers.mainThread())
    ...
    protected void toParentDir() {
        ...
        mSubscription = mService.parentDirectory(mClient)
                                .doOnSubscribe(mView::showLoading)
                                .doAfterTerminate(mView::hideLoading)
                                .subscribeOn(Schedulers.io())
                                .observeOn(AndroidSchedulers.mainThread())
    ...
    protected void rootDir() {
        ...
        mSubscription = mService.setWorkingDirectory(mClient, mServer.getPath())
                                .doOnSubscribe(mView::showLoading)
                                .doAfterTerminate(mView::hideLoading)
                                .subscribeOn(Schedulers.io())
                                .observeOn(AndroidSchedulers.mainThread())
    ...
    // GET и SET параметров сервера
    public FtpServer getFtpServer() { return mServer; }
    public void setFtpServer(FtpServer server) { mServer = server; }
    ...
    protected void newFolder(@NonNull String name) {
        ...
        if (!name.equals(".") && !name.equals("..") && !name.equals("*")) {
            mSubscription = mService.createDirectory(mClient, name)
                                    .doOnSubscribe(mView::showLoading)
                                    .doAfterTerminate(mView::hideLoading)
                                    .subscribeOn(Schedulers.io())
                                    .observeOn(AndroidSchedulers.mainThread())
    ...
    protected void newName(String oldName, String newName) {
        ...
        mSubscription = mService.renameFile(mClient, oldName, newName)
                                .doOnSubscribe(mView::showLoading)
                                .doAfterTerminate(mView::hideLoading)
                                .subscribeOn(Schedulers.io())
                                .observeOn(AndroidSchedulers.mainThread())
