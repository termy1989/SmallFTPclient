...
public class LocalStoragePresenter {
    ...
    // конструктор
    public LocalStoragePresenter(@NonNull LocalStorageView view, Context context, String path) {
        mView = view;
        mContext = context;
        mCurrentDir = path;
        mCachedList = null;
    ...
    public void refresh(String path) {
        ...
        else if (FilesProvider.provideFilesService().isRoot(mContext, mCurrentDir)
                    && FilesProvider.provideFilesService().isSDcard(path)
        ...
        // обычное обновление
                    ...
                    || (new File(path).exists())) {
            mCachedList = FilesProvider.provideFilesService().getListFiles(mCurrentDir);
            showFiles();
        }
    ...
    private void showFiles() {
        ...
        if (!FilesProvider.provideFilesService().isRoot(mContext, mCurrentDir)) {
    ...
    public boolean createNewFolder(String name) {
        ...
        return FilesProvider.provideFilesService().mkdir(mContext, new File(mCurrentDir, name));
    }

    // переименование элемента
    public boolean renameFile(String name, @NonNull LocalFile file) {

        return FilesProvider.provideFilesService().rename(mContext, new File(file.getPath()),
                                                                    new File(mCurrentDir, name));
